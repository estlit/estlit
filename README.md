### Academic Publications & Books

#### Books

* **[AI NPU System Design with Python and Verilog](https://www.amazon.com/dp/B0GLQVJWMK)** (2026)
  *Amazon Best Seller #2 in Microprocessor Design | #2 in Compiler Design | #4 in Microprocessors & System Design | #1 Amazon New Release in Microprocessor Design | #1 New Release in Compiler Design*

* **[SemiconductorSchool](https://www.amazon.com/dp/B0B8GYN5CL)** (2022)
  *#1 Amazon New Release in Semiconductor*

---

#### Academic Publications

* **FPGA Implementation of a Deterministic and Bit-Accurate Quad-Core Quad-Cycle Systolic Ensemble NPU for Edge Inference**
  *International Journal of Internet, Broadcasting and Communication (IJIBC), Vol.18 No.2, pp.110–121, 2026*
  DOI: https://doi.org/10.7236/IJIBC.2026.18.2.110

---

#### Open Hardware Projects

* **[EdgeChipLab_CPU_v1 — Successfully Passed RISC-V RV32I ACT v3.0](https://github.com/estlit/EdgeChipLab_RV32I_Compliance_Report)**

* **[EdgeChipLab GitHub](https://github.com/estlit)**

---

#### Research Areas

* RISC-V CPU Architecture & Architectural Verification
* RTL-based AI NPU Design
* FPGA-based Edge AI Systems
* Bit-Accurate Python + Verilog Co-Verification
* Hardware/Software Co-Design for Semiconductor Education

---

#### Contact

**Official Email:** [roger@ssu.ac.kr](mailto:roger@ssu.ac.kr)
**Book Inquiries:** [estlit@daum.net](mailto:estlit@daum.net)
